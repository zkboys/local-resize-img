(function () {
    ...
    var ImageCompresser = {
        ...
        getImageBase64: function (img, _opts) {
            ...
            var opts = {
                maxW: _opts.maxW || 800, //目标宽
                maxH: _opts.maxH || 800, //目标高
                quality: _opts.quality || 0.8, //目标jpg图片输出质量
                orien: _opts.orien || 1
            };
            //获取配置
            var maxW = opts.maxW, //图片最大限制
                ...
                w, h; //图片目标大小
            //ios平台针对大图做抽值处理
            if (os.ios && ImageCompresser.isIosSubSample(img)) {
                _w = _w / 2;
                _h = _h / 2;
            }
            //获取最终生成图片大小
    ...
    function resizeImg(options) {
        ...
        var filesIndex = 0;//哨兵变量？
        readFile(files);
        function readFile(files) {
            if (filesIndex == files.length) {
                options.input.val('');//处理完成之后，清空input值，这样可以连续上传相同的图片
                return;
            }
            ...
            reader.onload = function (e) {
                ...
                if (file.type == 'image/jpeg') {
                    ...
                    if (jpg.tiff && jpg.tiff.Orientation) {
                        //设置旋转
                        orientation = jpg.tiff.Orientation.value;
                    }
                ...
                img.onload = function (e) {
                    ...
                    if (typeof(options.input.attr("data-maxCount")) != "undefined") {
                        ...
                        if (maxCount) {
                            maxCount = maxCount - 1;
                            options.input.attr('data-maxCount', maxCount);
                        }
                    }
                    var result = {
                        base64: base64,
                    ...
                    // 执行后函数
                    success.call(options.input, result);
                    readFile(files);
                };
            };
        }
    ...
    (function ($, window, document, undefined) {
        $.fn.localResizeImg = function (options) {
            ...
            el.on('change', function (e) {
                ...
                var maxCount = 9;
                if (typeof(el.attr("data-maxCount")) != "undefined") {
                    maxCount = el.attr('data-maxCount');
                    maxCount = maxCount ? parseInt(maxCount) : maxCount;
                }
                if (before) {
                    var result = before.call(el, {
                        files: files,
                        maxCount: maxCount
                    });
                    if (result === false) {
